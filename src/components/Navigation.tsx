    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-md border-b border-border/20">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          {/* Logo */}
          <div className="font-bold text-xl bg-gradient-crimson bg-clip-text text-transparent">
            YKG
          </div>
          ...
          <div className="md:hidden border-t border-border/20 py-4 animate-fade-in">
            {navItems.map((item) => (
              <button
                ...
                onClick={() => scrollToSection(item.id)}
                className={cn(
                  "block w-full text-left px-4 py-2 text-sm font-medium transition-colors hover:text-primary",
                  activeSection === item.id ? "text-primary" : "text-muted-foreground"
                )}
              >
                {item.label}
